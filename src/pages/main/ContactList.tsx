...
export default function ContactList({
    ...
    onSelectEntities: (entities: Entity[]) => void;
    ...
    return (
        <Scrollbar>
            <List
                ...
                dense
            >
                {recentContacts.map((item, idx) => {
                    const selected = !!selectedEntities.find((value) =>
                        equal(value, item.entity),
                    );
                    const select = () =>
                        onSelectEntities(
                            multiple
                                ? selected
                                    ? selectedEntities.filter(
                                          (value) => !equal(value, item.entity),
                                      )
                                    : [...selectedEntities, item.entity]
                                : [item.entity],
                        );
                    return (
                        <ListItem
                            ...
                            sx={{
                                "& > *": {
                                    height: showLatestMessage ? 72 : undefined,
                                },
                            }}
                        >
                            <ListItemButton
