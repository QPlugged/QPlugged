...
export default function Login() {
	...
	useEffect(() => {
		api.login
			.getAccountList()
			.then((value) =>
				setAccountList(value.filter((account) => account.quickLoginSupported)),
			);
	}, [api]);
						...
						{accountList ? (
							<>
								{accountList.length !== 0 ? (
									accountList.map((account) => (
										<ListItem
											...
											disablePadding
											onClick={() => loginWithUin(account.uin)}
											secondaryAction={
												<IconButton disabled={loggingIn}>
													<Clear />
												</IconButton>
											}
										>
											<ListItemButton disabled={loggingIn}>
												<ListItemIcon>
													<Avatar
														src={account.avatar}
														alt={account.name}
													...
													>
														{account.name.slice(0, 1) || null}
													</Avatar>
												</ListItemIcon>
												<ListItemText
													primary={account.name ? `${account.name}` : "未知"}
								...
								<ListItem disablePadding>
									<ListItemButton
										disabled={loggingIn}
										onClick={() => api?.login.showLoginWindow()}
									>
										<ListItemIcon>
											<Key />
										</ListItemIcon>
