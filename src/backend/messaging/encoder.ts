...
export function encodeFaceElement(ele: any): MessageElementFace {
	return {
		...
		faceBigId: ele.faceElement.stickerId && parseInt(ele.faceElement.stickerId),
		raw: ele,
	};
}
...
export function encodeUser(raw: any): User {
	return {
		...
		bio: raw.longNick,
		sex:
			{ 1: "male", 2: "female", 255: "unset", 0: "unset" }[raw.sex as number] ||
			raw.sex,
		raw: raw,
	};
}
...
export function encodeGroup(raw: any): Group {
	return {
		...
		name: raw.groupName,
		role:
			{ 4: "master", 3: "moderator", 2: "member" }[raw.memberRole as number] ||
			raw.memberRole,
